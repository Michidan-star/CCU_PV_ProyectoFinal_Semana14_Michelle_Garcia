...
namespace Erdyka.Api.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> CrearUsuario([FromBody] CreacionUsuarioDto dto)
        {
            ...
            // Validar que el rol exista en la base de datos antes de crear
            var role = await _context.Roles.FindAsync(dto.RolId);
            if (role == null)
        ...
        public async Task<IActionResult> ActualizarUsuario(int id, [FromBody] CreacionUsuarioDto dto)
        {
            ...
            // Validar que el RolId proporcionado existe
            if (dto.RolId <= 0)
                return BadRequest(new { mensaje = "RolId inválido" });
